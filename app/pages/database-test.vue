<template>
  <div class="min-h-screen bg-gray-50 p-8">
    <div class="max-w-4xl mx-auto">
      <h1 class="text-3xl font-bold text-gray-900 mb-8">🧪 Prueba de Base de Datos</h1>
      
      <!-- Estado de la base de datos -->
      <UCard class="mb-6">
        <template #header>
          <h2 class="text-xl font-semibold">Estado de la Base de Datos</h2>
        </template>
        
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            <span>Inicializada:</span>
            <UBadge :color="isReady ? 'green' : 'red'">
              {{ isReady ? 'Sí' : 'No' }}
            </UBadge>
          </div>
          ...
          <div class="flex items-center justify-between">
            <span>Cargando:</span>
            <UBadge :color="isLoading ? 'yellow' : 'green'">
              {{ isLoading ? 'Sí' : 'No' }}
            </UBadge>
          </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <UButton @click="testDatabase" :loading="isLoading" size="lg" color="blue">
          <UIcon name="i-heroicons-database" />
          Probar Base de Datos
        </UButton>
        
        <UButton @click="testConfig" :loading="isLoading" size="lg" color="green">
          <UIcon name="i-heroicons-cog-6-tooth" />
          Probar Configuración
        </UButton>
        
        <UButton @click="testAccounts" :loading="isLoading" size="lg" color="purple">
          <UIcon name="i-heroicons-banknotes" />
          Probar Cuentas
        </UButton>
        
        <UButton @click="testCategories" :loading="isLoading" size="lg" color="orange">
      ...
      <UCard v-if="results.length > 0">
        <template #header>
          <h2 class="text-xl font-semibold">Resultados de las Pruebas</h2>
        </template>
        
        <div class="space-y-4">
          <div v-for="(result, index) in results" :key="index" class="p-4 border rounded-lg">
            <div class="flex items-center justify-between mb-2">
              <h3 class="font-medium">{{ result.test }}</h3>
              <UBadge :color="result.success ? 'green' : 'red'">
      ...
      <UCard v-if="logs.length > 0" class="mt-6">
        <template #header>
          <h2 class="text-xl font-semibold">Logs del Sistema</h2>
        </template>
        
        <div class="space-y-2">
          <div v-for="(log, index) in logs" :key="index" class="text-sm font-mono">
            <span class="text-gray-500">[{{ log.time }}]</span>
            <span :class="log.type === 'error' ? 'text-red-500' : 'text-green-500'">
              {{ log.message }}
            </span>
          </div>
        </div>
...
// Composables
const { isReady, isLoading, error, initialize, query, execute } = useDatabase()
